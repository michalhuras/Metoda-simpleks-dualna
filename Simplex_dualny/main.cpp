...
void krok_dualny (tablica_struct tab,tablica_struct tab2)
{
    unsigned w=tab.w,k=tab.k;
    int n,mini=99;
    for (int i=2;i<w;i++)
    {
        if (tab.tablica[i][1]<mini)
        {
            mini=tab.tablica[i][1];
            n=i;
        }
    }
    float tab_iloczyn[k-2];
    for (int i=0;i<k-2;i++)
    {
        tab_iloczyn[i]=0;
    }
    tab_iloczyn[0]=tab.tablica[1][2]/tab.tablica[n][2];
    float iloczynmaks=tab_iloczyn[0];
    int m=2;
    for (int i=3;i<k;i++)
    {
        tab_iloczyn[i-2]=tab.tablica[1][i]/tab.tablica[n][i];
        if (tab_iloczyn[i-2]>iloczynmaks && tab.tablica[n][i]<0)
        {
            iloczynmaks=tab_iloczyn[i-2];
            m=i;
        }
    }
    int zerowa[w][k];
